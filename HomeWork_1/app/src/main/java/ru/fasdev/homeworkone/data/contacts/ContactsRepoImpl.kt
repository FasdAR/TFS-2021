...
class ContactsRepoImpl(context: Context) : ContactsRepo {
    ...
    override fun getContacts(): List<Contact> {
        ...
        cursor?.use { it ->
            while (it.moveToNext()) {
                val nameContact = it.getValue(DISPLAY_NAME_COLUMN)
                val lookUpContact = it.getValue(LOOK_UP_COLUMN)

                arrayContacts.add(
                    Contact(
                        lookUpContact, nameContact,
    ...
    private fun queryOneValue(lookupKey: String, uri: Uri, projection: String): String {
        val cursor = contentResolver.query(
            uri, arrayOf(projection), "${ContactsContract.Data.LOOKUP_KEY} = ?",
